...
const COMMANDS = [
  ...
  { cmd: "elyon job status --id <id>", desc: "Vérifier le statut d'un travail" },
  { cmd: "elyon template render --input doc.html --output result.pdf", desc: "Rendre un template HTML en PDF" },
  { cmd: "elyon config set --key API_KEY --value <key>", desc: "Configurer votre clé API" },
  { cmd: "elyon batch submit --file jobs.json", desc: "Soumettre un lot d'impression" },
  { cmd: "elyon logs tail --level error", desc: "Suivre les logs en temps réel" },
  { cmd: "elyon version", desc: "Afficher la version installée" },
]
...
const FEATURES = [
  {
    ...
    desc: "Une seule commande pour installer le CLI sur macOS, Linux et Windows. Mises à jour automatiques via notre registre.",
  },
  {
    icon: Settings,
    title: "Configuration flexible",
    desc: "Fichier de configuration YAML, variables d'environnement ou flags en ligne de commande. Choisissez votre méthode.",
  },
  {
    icon: FileCode,
    title: "Mode pipe",
    desc: "Utilisez le CLI dans vos pipelines shell et scripts bash. Entrée/sortie standard pour une intégration maximale.",
  },
  {
    icon: RefreshCw,
    title: "CI/CD natif",
    desc: "Des actions GitHub, GitLab CI et Jenkins prêtes à l'emploi. Intégrez l'impression dans vos pipelines de déploiement.",
  },
  {
    icon: Users,
    title: "Mode équipe",
    desc: "Partagez la configuration et les templates entre collaborateurs. Contrôle d'accès par équipe et projet.",
  },
  {
    icon: BookOpen,
      ...
      <section className="relative overflow-hidden px-4 py-24">
        ...
        <div className="relative z-10 mx-auto max-w-[1220px]">
          ...
          <p className="mt-4 text-lg leading-relaxed text-muted-foreground max-w-2xl">
            Une suite d'outils en ligne de commande pour intéragir avec
            votre infrastructure d'impression. Idéal pour l'automatisation,
          ...
          <div className="rounded-2xl border border-border/50 bg-gradient-to-br from-primary/[0.06] via-background/50 to-background/50 p-8 sm:p-12">
            <h2 className="text-2xl font-bold tracking-tight sm:text-3xl">Prêt à installer ?</h2>
            <p className="mt-3 max-w-xl text-base leading-relaxed text-muted-foreground">
              Une seule ligne de commande et vous êtes opérationnel.
